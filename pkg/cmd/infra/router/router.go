...
func (o *RouterSelection) Bind(flag *pflag.FlagSet) {
	...
	flag.BoolVar(&o.EndpointAddressValidation, "endpoint-address-validation", isTrue(env("ENDPOINT_ADDRESS_VALIDATION", "false")), "If set, endpoint addresses are validated against restricted IP ranges and FQDN-typed EndpointSlices are rejected. Defaults to false. Should be true when hostNetwork is being used.")
...
func (o *RouterSelection) NewFactory(routeclient routeclientset.Interface, projectclient projectclient.ProjectInterface, kc kclientset.Interface) *controllerfactory.RouterControllerFactory {
	factory := controllerfactory.NewDefaultRouterControllerFactory(routeclient, projectclient, kc, o.WatchEndpoints, o.EndpointAddressValidation)
